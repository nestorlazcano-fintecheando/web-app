...
export class JlgLoansAccountComponent implements OnInit {
    ...
    this.jlgLoanForm.get('productId').valueChanges.subscribe( async productId => {
      if (productId) {
        await this.loansService.getLoansAccount(groupId, productId).toPromise().then( (data: any) => {
          ...
          for (const key in this.clientMembers) {
            this.clientMembers[ key ].principal = data.memberVariations[ this.clientMembers[ key ].id ].principal;
            this.clientMembers[ key ].charges = this.charges;
          }
          return;
        } );
      }
  ...
  removeCharge(id: string, item: any) {
    item.charges = item.charges.filter( (item: any) => item.chargeId !== id );
  }

  /** Toggle all checks */
  toggleSelects() {
    for (const key in this.clientMembers) {
      this.clientMembers[ key ].selected = this.selectAllItems;
    }
  }
  ...
  buildRequestData(): any[] {
    ...
    const requestData = [];
    for (const key in membersSelected) {
      requestData.push({
        body: JSON.stringify( this.setData( membersSelected[ key ] ) ),
        method: 'POST',
        relativeUrl: 'loans',
        requestId: key
      });
